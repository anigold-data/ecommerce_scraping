...
class AsdaScraper(BaseScraper):
    """ASDA-specific scraper implementation"""

    def __init__(self):
        super().__init__('ASDA', base_delay=3, jitter=1.5)
    ...
    def extract_product_data(self, soup, url):
        ...
        # Product name
        try:
            product_data['name'] = soup.select_one('h1.pdp-main-details__title').get_text(strip=True)
        ...
        # Current price
        try:
            price_container = soup.select_one('.pdp-main-details__price')
            if price_container:
        ...
        # Original price
        try:
            was_price = soup.select_one('.pdp-main-details__was-price')
            if was_price:
        ...
        # Availability
        try:
            availability = soup.select_one('.pdp-stock-info__availability')
            if availability:
        ...
        # Product ID (ASDA item number)
        try:
            product_id_elem = soup.select_one('.pdp-product-info__product-code')
            if product_id_elem:
        ...
        # Rating
        try:
            rating_elem = soup.select_one('.pdp-reviews__rating')
            if rating_elem:
        ...
        # Reviews
        try:
            reviews_elem = soup.select_one('.pdp-reviews__count')
            if reviews_elem:
        ...
        # Brand
        try:
            brand_elem = soup.select_one('.pdp-product-info__brand')
        ...
        # Features
        try:
            features = soup.select('.pdp-product-details__description li')
        ...
        # Unit price
        try:
            unit_price_elem = soup.select_one('.co-product__price-per-uom')
        ...
        # Smart Price (ASDA's value range indicator)
        try:
            smart_price = soup.select_one('.pdp-main-details__smart-price')
            product_data['smart_price'] = True if smart_price else False
        except Exception as e:
            self.logger.warning("Could not determine if product is Smart Price")
            product_data['smart_price'] = None
